...
PASSING='{"contexts":{"nodes":[{"__typename":"CheckRun","status":"COMPLETED","conclusion":"SUCCESS"}]}}'
FAILING='{"contexts":{"nodes":[{"__typename":"CheckRun","status":"COMPLETED","conclusion":"FAILURE"}]}}'
PENDING='{"contexts":{"nodes":[{"__typename":"StatusContext","state":"PENDING"}]}}'
NO_CHECKS='null'
NO_LABELS='[]'
...
review_open() {
  ...
  printf '%s %s\n' acme/etl "$(pr acme/etl 15 'ETL: nightly loader for the team' team-loader teammate OPEN 2020-01-01T00:00:00Z null null "$NO_LABELS" "$NO_REVIEWS" "$PASSING")"

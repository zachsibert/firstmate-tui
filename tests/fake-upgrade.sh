#!/usr/bin/env bash
# tests/fake-upgrade.sh - stands in for <prefix>/bin/fm-board.sh when the
# Settings page runs an upgrade in tests. tests/fm-board.test.sh copies it to
# a fake install prefix as bin/fm-board.sh and points the board at that prefix
# with --install-root, so `bash <prefix>/bin/fm-board.sh upgrade ...` reaches
...
echo "install: downloading fm-board-v0.2.0.tar.gz from acme/fm-board-test release v0.2.0"

...
options:
  ...
  --no-mouse             ignore the mouse and leave the terminal's own text selection
                         alone (default: click selects, double-click is enter,
                         right-click opens the row's menu, the wheel scrolls)
  --mouse <list>         with --render-once: mouse events, applied in order with --keys
                         (comma or space separated): click:X,Y  dblclick:X,Y  rclick:X,Y
                         wheel:up:X,Y  wheel:down:X,Y, X and Y the cell from 0 at the
                         top-left; any other token is a key, so "rclick:12,5 down enter"
                         opens a row's menu and chooses its second item
...
export function parseMouseToken(token) {
  const m = /^(click|dblclick|rclick|wheel:(?:up|down)):(\d+),(\d+)$/.exec(token);
  if (!m) {
    if (/^(click|dblclick|rclick|wheel)(:|$)/.test(token)) throw new Error(`--mouse: bad event "${token}" (want click:X,Y, dblclick:X,Y, rclick:X,Y, wheel:up:X,Y or wheel:down:X,Y)`);
    return null;
  }
  const x = Number(m[2]);
  const y = Number(m[3]);
  switch (m[1]) {
    ...
    case 'rclick':
      return [{ type: 'down', button: 'right', x, y }];
    default:
...
export function parseArgs(argv, env = {}) {
  const opts = {
    ...
    // and [{ kind: 'mouse', events }], so a test can right-click, then press.
    inputs: [],
    expand: [],
    help: false,
  };
  ...
  while (args.length) {
    ...
    switch (a) {
      ...
      case '--mouse':
        ...
        for (const token of need(a).match(/(?:click|dblclick|rclick|wheel:(?:up|down)):\d+,\d+|[^\s,]+/g) || []) {

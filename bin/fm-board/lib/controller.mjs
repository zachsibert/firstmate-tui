...
// do what enter does there; a right-click on a row selects it and opens the
// menu of its actions (rowActions), each line the key and what it does, and
// choosing an item presses that key, so the menu has no handler of its own;
// the wheel moves the selection WHEEL_ROWS rows in the focused pane. While
// the menu is open, up/down/j/k and the wheel move the highlight, enter or a
// left click on an item runs it, esc or a click outside closes it, and any
...
import { hitTest, insideMenu, menuItemAt, PANES } from './layout.mjs';
...
// The right-click menu of one row: [{ key, label }], one line per action the
// keys offer on that row right now, built by asking keyAction what each key
// would do so the menu can never list an action the key would refuse. Where
// enter duplicates l or h (a group row) the dedicated key is the one shown.
// X (unhide the pane) appears only when the pane holds hidden rows and H only
// when the board holds hidden rows or is already showing them.
const MENU_KEYS = ['l', 'h', 'enter', 'x', 'X', 'H'];
const MENU_RANK = { enter: 0, l: 1, h: 2, x: 3, X: 4, H: 5 };

function menuLabel(action, view) {
  switch (action.type) {
    case 'open':
      return 'open PR';
    case 'focus':
      return 'focus herdr pane';
    case 'view':
      return 'view report';
    case 'expand':
      return 'expand group';
    ...
    case 'unhide':
      return 'unhide row';
    case 'unhide-pane':
      return 'unhide all in pane';
    case 'toggle-hidden':
      return view.showHidden ? 'hide hidden rows' : 'show hidden rows';
    default:
      return null;
  }
}
...
function actionSignature(action) {
  switch (action.type) {
    ...
    case 'hide':
    case 'unhide':
      return `${action.type}:${action.row.hideKey}`;
    default:
      return action.type;
  }
}

export function rowActions(model, view, paneIdx, rowIdx) {
  const pane = model.panes[paneIdx];
  const row = pane && !pane.hidden ? pane.rows[rowIdx] || null : null;
  if (!row) return [];
  const probe = { ...view, pane: paneIdx, row: rowIdx };
  const hiddenOnBoard = (model.meta && model.meta.hiddenRows > 0) || Boolean(view.showHidden);
  const seen = new Set();
  const items = [];
  for (const key of MENU_KEYS) {
    const action = keyAction(model, probe, key);
    const label = menuLabel(action, view);
    if (!label) continue;
    if (action.type === 'unhide-pane' && !(pane.hiddenCount > 0)) continue;
    if (action.type === 'toggle-hidden' && !hiddenOnBoard) continue;
    const sig = actionSignature(action);
    if (seen.has(sig)) continue;
    seen.add(sig);
    items.push({ key, label });
  }
  return items.sort((a, b) => MENU_RANK[a.key] - MENU_RANK[b.key]);
}
...
// that row), open-menu (a right-click on a row), wheel, none.
export function mouseAction(model, view, ev) {
  ...
  if (ev.type !== 'down') return { type: 'none' };
  ...
  if (ev.button !== 'left' && ev.button !== 'right') return { type: 'none' };
  if (hit.kind !== 'row') return { type: 'select', pane: hit.pane, row: hit.pane === view.pane ? view.row : 0 };
  if (ev.button === 'right') return { type: 'open-menu', pane: hit.pane, row: hit.row, x: ev.x, y: ev.y, items: rowActions(model, view, hit.pane, hit.row) };
...
function moveMenu(view, dir) {
  const n = view.menu.items.length;
  view.menu.index = Math.max(0, Math.min(n - 1, view.menu.index + dir));
}

// Run menu item i: close the menu, put the selection back on the row the menu
// was opened for (a refresh may have moved it), then press the item's key.
function runMenuItem(ctx, i) {
  const { view } = ctx;
  const menu = view.menu;
  view.menu = null;
  const item = menu.items[i];
  if (!item) return;
  const pane = ctx.model.panes[menu.pane];
  if (pane && !pane.hidden) {
    const idx = menu.hideKey ? pane.rows.findIndex((r) => r.hideKey === menu.hideKey) : -1;
    view.pane = menu.pane;
    view.row = idx >= 0 ? idx : menu.row;
    clampSelection(ctx);
  }
  handleKey(ctx, item.key);
}
  ...
  if (view.menu) {
    switch (key) {
      ...
      case 'j':
        ...
        return;
      case 'enter':
        runMenuItem(ctx, view.menu.index);
        return;
      case 'escape':
        view.menu = null;
        return;
      default:
        view.menu = null; // and the key means what it always means
    }
  }
...
export function handleMouse(ctx, ev) {
  ...
  if (view.menu) {
    const frame = view.frame || { cols: 0, rows: 0 };
    if (ev.type === 'wheel') {
      moveMenu(view, ev.dir === 'up' ? -1 : 1);
      return;
    }
    if (insideMenu(view.menu, frame.cols, frame.rows, ev.x, ev.y)) {
      const i = ev.button === 'left' ? menuItemAt(view.menu, frame.cols, frame.rows, ev.x, ev.y) : -1;
      if (i >= 0) runMenuItem(ctx, i);
      return;
    }
    view.menu = null;
    if (ev.button !== 'right') return; // a left click elsewhere only closes; a right-click goes on to its row
  }
...
function applyAction(ctx, action) {
  ...
  switch (action.type) {
    ...
    case 'open-menu': {
      ...
      view.lastClick = null;
      clamp();
      const row = selectedRow(ctx.model, view);
      if (action.items.length && row) view.menu = { x: action.x, y: action.y, items: action.items, index: 0, pane: view.pane, row: view.row, hideKey: row.hideKey };
      return;
    }
    case 'wheel': {

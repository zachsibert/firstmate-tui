...
M1, the read-only board, is implemented in this repository, plus the M1b follow-on (`enter` opens a PR in the browser, In flight groups secondmate work by home, and Needs you lists the main home only) and M2: `enter` on a Findings row opens the report in a terminal viewer, lost panes show in red, `x` / `X` / `H` hide and unhide rows, `1`-`5` / `0` hide and show panes (each title carries its key; with all five hidden a key page replaces the grid), `r` refreshes the snapshot and the live PR checks at once, and the mouse selects, double-clicks as `enter`, right-clicks for a menu of a row's actions and scrolls with the wheel (`--no-mouse` turns it off). The scout report that grounds the plan is in [`docs/scout-report-2026-09-16.md`](docs/scout-report-2026-09-16.md): data availability per pane, herdr capabilities, what is reusable from yimbot as a pattern, stack choice, and the four-milestone plan. Answering decisions, toasts and the findings watermark are later milestones.
...
| `--no-mouse` | ignore the mouse: no click, double-click, right-click menu or wheel, and mouse reporting is never switched on, so the terminal's own text selection works as usual. Default: mouse on (see [Mouse](#mouse)) |
| `--mouse <list>` | with `--render-once`: mouse events, applied in order with `--keys` (comma or space separated): `click:X,Y`, `dblclick:X,Y`, `rclick:X,Y`, `wheel:up:X,Y`, `wheel:down:X,Y`, X the column and Y the line, both from 0 at the top-left cell; any other token is a key, so `rclick:30,11 down enter` opens a row's menu and runs its second item. The events take the same path as the terminal's, measured against the frame the app would have drawn |
...
Everything the keys do to a row, the mouse does too. Clicking is the same as navigating there with the keys; nothing else changes.
...
| right-click on a row | select it and open a menu at the pointer listing the actions the keys offer on that row, each with its key: `enter  open PR`, `enter  focus herdr pane`, `enter  view report`, `l  expand group`, `h  collapse group`, `x  hide row` (`x  unhide row` on a hidden row shown by `H`), `X  unhide all in pane` when the pane holds hidden rows, `H  show hidden rows` when the board does. Only the actions valid for that row and pane appear. Arrow keys, `j`/`k` or the wheel move the highlight; `enter` or a click on an item runs it through the very same handler as the key; `esc` or a click anywhere else closes the menu; any other key closes it and then does what it always does. A right-click on a pane title or empty space only focuses the pane |
| wheel | move the selection three rows in the focused pane, whichever pane the pointer is over |

No drag, no resizing, and the title line and footer ignore clicks. `--no-mouse` turns all of it off.

Two things have to hold for the mouse to reach the board. Your terminal must support mouse reporting: the board asks the terminal to report clicks when it starts and stops asking when it exits, and while it is asking, a plain drag no longer selects text, so hold your terminal's text-selection modifier while dragging, or start with `--no-mouse` and use the keys. Inside herdr, herdr must pass mouse events to the pane. `herdr --default-config` (herdr 0.8.2) documents the two settings involved: `mouse_capture = true`, the default, captures the mouse for herdr's own UI but, in its words, pane apps "can still receive mouse when they request it", which the board does; and `right_click_passthrough_modifier` decides whether a right-click held or dragged goes to herdr's pane menu or to the pane app. If herdr's own pane menu opens on a right-click instead of the board's, that setting is the one to look at.
...
bin/fm-board/lib/layout.mjs  pure: pane heights, columns, width breakpoints, the mouse hit test and the menu box
bin/fm-board/lib/render.mjs  pure: model + view state -> frame lines (plus what each line shows, for the mouse) and the help and menu overlays
bin/fm-board/lib/herdr.mjs   herdr CLI calls and the socket subscription client
bin/fm-board/lib/controller.mjs  pure key and mouse semantics (move, expand/collapse, open, focus, view, hide, panes, refresh; click, double-click, the right-click menu, wheel) shared by the app and --keys / --mouse
